...
public class Generator {
    ...
    private static void handleMixin(Parsed parse) {
    ...
    private static <T> T findInheritanceProperty(ClassOrInterfaceDeclaration spec, PrototypeData properties, BiFunction<ClassOrInterfaceDeclaration, PrototypeData, T> func) {
        ...
        if (isNull(data)) {
            for (var type : spec.getExtendedTypes()) {
                ...
                if (nonNull(parse)) {
                    data = findInheritanceProperty(parse.getDeclaration(), parse.getProperties(), func);
                    if (nonNull(data)) {
                        break;
                    }
                }
        ...
        for (var member : modifier.getMembers()) {
            if (member.isMethodDeclaration()) {
                ...
                if (CollectionsHandler.isCollection(method.getType())) {
                    ...
                    addModifierFromModifier(parsed.getDeclaration(), parentModifier, method, null, parentProperties.getModifierName(), false);
                }
            }
        }
    }
