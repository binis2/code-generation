...
public class RequiredArgsConstructorEnricherHandler extends BaseArgsConstructorEnricherHandler implements RequiredArgsConstructorEnricher {


    @Override
    protected Stream<CGVariableDecl> applyFieldsFilter(Stream<CGVariableDecl> stream) {
        return stream.filter(CGVariableDecl::isFinal)
                .filter(v -> !v.isStatic())
