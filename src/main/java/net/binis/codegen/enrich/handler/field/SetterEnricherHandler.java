...
import static net.binis.codegen.compiler.CGFlags.PARAMETER;
...
public class SetterEnricherHandler extends BaseEnricher implements GetterEnricher {
    ...
    public void safeEnrichElement(ElementDescription description) {
        ...
        if (declaration instanceof CGClassDeclaration cls && !cls.isInterface()) {
            if (!cls.isAnnotation()) {
                cls.getDefs().stream()
                        ...
                        .toList().forEach(field -> {
                            var name = Helpers.getSetterName(field.getName().toString());
                            if (cls.getDefs().stream()
                                ...
                                createSetter(description.getElement(), name, field);
                            }
                        });
            }
            ...
            note("Setter is applicable only for classes and fields.", description.getElement());
        }
    }

    protected void createSetter(Element element, String name, CGVariableDecl field) {
        var maker = TreeMaker.create();
        ElementMethodUtils.addMethod(element, name, maker.Type(CGSymtab.voidType()), PUBLIC, List.of(maker.VarDef(maker.Modifiers(PARAMETER), field.getName(), field.getVarType(), null)),
                createBlock(createStatement(maker.Assign(createFieldAccess(maker, "this." + field.getName()), maker.Ident(field.getName())))));
    }
